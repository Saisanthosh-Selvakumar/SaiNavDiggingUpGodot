...
func _draw():
	...
	var blink_scale := 1.0 if not is_blinking else 0.08

	# Glow behind eye
	for i in 4:
		var glow_r = EYE_RADIUS + 30 + i * 15
		var alpha = 0.08 - i * 0.015
		draw_circle(Vector2.ZERO, glow_r, Color(1.0, 0.1, 0.0, alpha * (1.0 + anger_level)))

	# Eyelids (top and bottom)
	var lid_color := Color(0.04, 0.0, 0.0)
	if blink_scale < 1.0:
		# Top lid comes down
		var lid_extent = EYE_RADIUS * (1.0 - blink_scale)
		draw_rect(Rect2(-EYE_RADIUS - 10, -EYE_RADIUS - 10, (EYE_RADIUS + 10) * 2, lid_extent + EYE_RADIUS + 10), lid_color)
		draw_rect(Rect2(-EYE_RADIUS - 10, EYE_RADIUS * blink_scale, (EYE_RADIUS + 10) * 2, EYE_RADIUS * (1.0 - blink_scale) + 10), lid_color)

	if blink_scale < 0.2:
		return

	# Sclera (white of eye) — slightly bloodshot in phase 3
	var sclera_color := Color(0.92, 0.88, 0.85)
	if phase == 3:
		sclera_color = Color(0.9, 0.7, 0.7)
	draw_circle(Vector2.ZERO, EYE_RADIUS, sclera_color)
	...
	# Iris
	draw_circle(pupil_dir * (EYE_RADIUS - IRIS_RADIUS) * 0.4, IRIS_RADIUS * blink_scale, iris_color)

	# Iris detail ring
	var iris_pos := pupil_dir * (EYE_RADIUS - IRIS_RADIUS) * 0.4
	draw_arc(iris_pos, IRIS_RADIUS * blink_scale * 0.8, 0, TAU, 48, Color(0.9, 0.3, 0.1, 0.4), 2.0)
	draw_arc(iris_pos, IRIS_RADIUS * blink_scale * 0.6, 0, TAU, 32, Color(1.0, 0.5, 0.1, 0.3), 1.5)

	# Pupil (slit-like when angry, round otherwise)
	var p_pos := iris_pos
	if phase < 3:
		draw_circle(p_pos, PUPIL_RADIUS * blink_scale, Color(0.02, 0.0, 0.0))
	else:
		# Slit pupil for phase 3
		draw_ellipse_approx(p_pos, Vector2(PUPIL_RADIUS * 0.35 * blink_scale, PUPIL_RADIUS * blink_scale))

	# Highlight glint
	draw_circle(p_pos + Vector2(-8, -10) * blink_scale, 5.0 * blink_scale, Color(1, 1, 1, 0.85))
	draw_circle(p_pos + Vector2(-5, -7) * blink_scale, 2.5 * blink_scale, Color(1, 1, 1, 0.5))

	# Eye outline
	draw_arc(Vector2.ZERO, EYE_RADIUS, 0, TAU, 64, Color(0.6, 0.1, 0.05, 0.7), 3.0)

func _draw_veins():
	var rng := RandomNumberGenerator.new()
	rng.seed = 42
	var vein_count := int(anger_level * 12) + 2
	for i in vein_count:
		var start_angle := rng.randf() * TAU
		var start_r := EYE_RADIUS * rng.randf_range(0.7, 0.98)
		var start := Vector2(cos(start_angle), sin(start_angle)) * start_r
		var end_r := rng.randf_range(0.2, 0.5) * EYE_RADIUS
		var end_angle := start_angle + rng.randf_range(-0.4, 0.4)
		var end_v := Vector2(cos(end_angle), sin(end_angle)) * end_r
		draw_line(start, end_v, Color(0.8, 0.1, 0.1, 0.3 * anger_level), 1.2)

func draw_ellipse_approx(center: Vector2, radii: Vector2):
	var points := PackedVector2Array()
	var steps := 32
	for i in steps + 1:
		var a := i * TAU / steps
		points.append(center + Vector2(cos(a) * radii.x, sin(a) * radii.y))
	draw_colored_polygon(points, Color(0.02, 0.0, 0.0))
